...
No broad wrapper APIs. Avoid reusable helpers like `sendSuperlogSpan`, `recordCounter`, `recordLog`, `startTelemetrySpan`, or `withTelemetry`. Acquire native tracers/meters/loggers at module scope and use the SDK's own APIs directly. In TypeScript/JavaScript, prefer the published `@superlog/otel-helpers` `withSpan` helper for bounded business spans when it avoids expanding a whole function into `startActiveSpan` plus `try` / `catch` / `finally`; do not use helpers around provider SDK calls that OpenInference/provider instrumentation can observe directly. If an edge runtime genuinely cannot load an upstream OTel SDK, keep the shim tiny, provider-neutral, and OTel-shaped: `tracer.startActiveSpan`, `span.setAttributes`, `SpanStatusCode`, `meter.createCounter`, `histogram.record`.
...
- **Costs — especially LLM costs.** If the project calls OpenAI / Anthropic / Google / any LLM provider, prefer provider instrumentation such as OpenInference where available so native SDK calls stay readable. Record additive dollar costs only when pricing is available, and avoid duplicating token counters already captured by provider instrumentation. Use current semantic-convention-style attributes such as `gen_ai.provider.name`, `gen_ai.request.model`, `gen_ai.usage.input_tokens`, `gen_ai.usage.output_tokens`, plus app-bounded `app.gen_ai.use_case` / `app.gen_ai.call_site` and `outcome`. Use histograms for per-call latency/duration, not as the only token/cost total.
